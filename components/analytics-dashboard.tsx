...
import { ArrowLeft, Eye, Monitor, Smartphone, Globe, ExternalLink, Calendar, Check, Copy } from "lucide-react"
...
export default function AnalyticsDashboard({ id }: { id: string }) {
  ...
  const shareUrl = `${process.env.NEXT_PUBLIC_APP_DOMAIN}/api/v/${id}`
  useEffect(() => {
          ...
          <Card className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-lg font-bold">{getTop(counters.devices)[0]?.[0] || 'N/A'}</p>
              </div>
            </div>
          </Card>
          ...
          <Card className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-lg font-bold">{getTop(counters.geo)[0]?.[0] || 'N/A'}</p>
              </div>
            </div>
          </Card>
          ...
          <Card className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-lg font-bold">{getTop(counters.referrals)[0]?.[0] || 'N/A'}</p>
              </div>
            </div>
          </Card>
        </div>
              ...
              {getTop(counters.browsers).map(([browser, count]) => (
                <div key={browser} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Progress value={(count / totalViews) * 100} className="w-20" />
          ...
          <Card className="p-6">
            ...
            <div className="space-y-3">
              {getTop(counters.referrals).map(([referral, count]) => (
                <div key={referral} className="flex items-center justify-between">
            ...
            {recent.slice(0, 10).map((view, index) => (
              <div key={index} className="flex items-center justify-between p-3 rounded-lg border border-border">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-medium">{new Date(view.ts).toLocaleString()}</p>
                    <p className="text-xs text-muted-foreground">
                      {view.browser} on {view.device} • {view.country} • {view.referer}
                    </p>
                  </div>
                </div>
